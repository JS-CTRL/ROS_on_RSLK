...
// *                                                                                                                *
// * Purpose: To control a TI RSLK via ROS                                                                          *
// *                                                                                                                *
// * Methods: This program will allow the user to:                                                                  *
// *            -Publish linear x and angular z cmd_vel commands to drive/move the RSLK                             *
// *            -Publish linear z commands to move a servo (used to control a pen attatchment)                      *
// *            -Subscribe to bumperStatus to monitor bumper button presses                                         *
// *            -Subscribe to motorSTatus to monitor left and right motor PWM values, direction and linear velocity *
// *            -Subscribe to tf to monitor RSLK theoretical and measured position relative to starting position    *
// ******************************************************************************************************************
...
float speed_ang=0.0, speed_lin=0.0, z_lin=0.0;
...
void setup() {
  ...
  digitalWrite(SERVO_PIN, LOW); //initialize SERVO_PIN to low
...
void loop() {
  ...
  if ( (currentMillis - previousMillis) >= interval) { //used as a noninterrupt delay
    
    
    
    penUp(z_lin); //updates pen condition via linear z value

    previousMillis = currentMillis;//update current time
...
//**********************************************************************************
//Function Name: penUp(float pos)
//Purpose: To turn a servo in order to raise or lower a pen 
//         to satisfy the incoming linear and angular velocity commands
//Method: Takes in linear z value. For a negative input the pen will lower and for a 
//        non negative input the pen will rise. A servos position is dependent on the 
//        delay for low pulse. Change values for specific application. Servo library 
//        not used because of interrupt interference with ROS
//**********************************************************************************
void penUp(float pos){
  if (pos < 0){
